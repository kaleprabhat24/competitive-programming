...
There is a new mobile game that starts with consecutively numbered clouds.  Some of the clouds are thunderheads and others are cumulus.  The player can jump on any cumulus cloud having a number that is equal to the number of the current cloud plus 
...
.  The number on each cloud is its index in the list so the player must avoid the clouds at indices 
...
. The game can be won with a minimum of 
