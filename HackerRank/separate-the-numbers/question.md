...
- 
 ...
 (i.e., each element in the sequence is 

 more than the previous element).

- *No 

 contains a leading zero.* For example, we can split 
...
. For each query, print whether or not the string is beautiful on a new line. If it is beautiful, print `YES x`, where 

 is the first number of the increasing sequence.  If there are multiple such values of 
...
Each of the next 
 ...
 lines contains an integer string 
...
, it starts with a zero so all possible splits violate the second condition.
...
, there are no possible splits because 
